...
function heroMatchesSearch(hero) {
    ...
    const haystack = [
        ...
        roleLabel,
        heroItems.map((item) => {
            return `${item.name} ${item.itemType} ${item.quality} ${item.armorSet}`;
        }).join(" ")
    ].join(" ").toLowerCase();
...
function buildRecommendationNotes(filteredItems) {
    ...
    if (strongest.length > 0) {
        notes.push({
            ...
            text: strongest.map((row) => {
                return `${row.itemType}: ${row.name} (${row.score})`;
            }).join(" | ")
        });
    }
    ...
    notes.push({
        ...
        text: "This reviews equipped gear for the focused hero. The next big parser update will add item-box gear so this can recommend exact replacements."
    });
...
function renderTable(filteredItems) {
    ...
    elements.tableCount.textContent = `${escapeText(focusedHeroName)} gear • ${formatNumber(filteredItems.length)} item${filteredItems.length === 1 ? "" : "s"} shown • ${escapeText(roleLabel)}`;
