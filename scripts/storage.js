...
function saveOptimizerData() {
    ...
    localStorage.setItem(storageKeys.optimizer, JSON.stringify(optimizerData));
}
...
function loadOptimizerData() {
    ...
    const savedData = safelyParseJson(localStorage.getItem(storageKeys.optimizer));

    if (!savedData) {
        return;
    }

    if (document.querySelector("#main-goal")) {
        document.querySelector("#main-goal").value = savedData.mainGoal || "leveling";
    }

    if (document.querySelector("#has-genie")) {
        document.querySelector("#has-genie").checked = savedData.hasGenie || false;
    }

    if (document.querySelector("#has-fish")) {
        document.querySelector("#has-fish").checked = savedData.hasFish || false;
    }

    if (document.querySelector("#has-seahorse")) {
        document.querySelector("#has-seahorse").checked = savedData.hasSeahorse || false;
    }

    if (document.querySelector("#has-propeller-cat")) {
        document.querySelector("#has-propeller-cat").checked = savedData.hasPropellerCat || false;
    }
    ...
    if (results) {
        results.innerHTML = `
            <div class="card-content">
                ...
                <p>Fill out the form and click “Get Recommendation.”</p>
            </div>
        `;
    }
}
